...
- ENT-A~C now provide a source-neutral enterprise identity/time envelope, versioned synthetic mapping
  profiles, source-span candidates, dirty-source disposition and a deterministic in-memory
  sync/reconciliation checkpoint. This proves fixture cursor resume, idempotency, bounded retry and
  tombstone/stale precedence, not transport or durability. There is still no Jira/Confluence adapter,
  real ACL inheritance, database checkpoint, canonical import, durable quarantine/resolution,
  retention or human-reviewed unstructured promotion path. Direct live company connection remains
  NO-GO.
